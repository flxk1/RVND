...
(:func:`rvnd.governance.decide_action` — gate × matrix × oversight ×
...
_MATCHER_ALL = "*"
_MARKER = "rvnd.hook"   # how we recognise OUR entry for idempotency/removal
...
def _hook_command() -> str:
    ...
    if which:
        return which
    return f"{sys.executable} -m rvnd.hook"
...
def _cli(argv: list[str]) -> int:
    ...
    ap.add_argument("--command", default=None,
                    help="override the command string written to settings.json "
                         "(default: absolute rvnd-hook, else 'python -m rvnd.hook')")
