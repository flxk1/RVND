...
# ---------------------------------------------------------------------------
# Folder identity — RETIRED to loomground-workspace, consumed via the seam.
# ---------------------------------------------------------------------------
# ``folder_hash`` / ``legacy_folder_hash`` / ``_filesystem_is_case_insensitive``
# used to be defined right here. Deciding when two paths are the same workspace
# is part of what a workspace IS, not part of what a mutation log does, so they
# moved to ``loomground_workspace.identity``.
#
# The re-export is NOT cosmetic: ten src modules and ten test modules import
# these names ``from .mutation_log``. ``legacy_folder_hash`` in particular has
# only three live references (``seal._resolve_log_dir``, ``_is_sealed`` below,
# and one test) and losing it fails SILENTLY — ``_is_sealed`` would return False
# for a workspace sealed under the legacy hash, and ``__init__`` would then
# recreate a plaintext log directory beside the ciphertext, which by the comment
# at ``_ensure_dir`` makes ``unseal`` refuse. Keep all three exported.
