...
Two pure-stdlib layers (no external engine, no cross-repo dependency):
  ...
  1. PROMPT-INJECTION tripwire over the text body — DELEGATED to ``rvnd.lock.injection_scan``
...
# ── Layer 1: prompt injection — DELEGATED to rvnd.lock.injection_scan (the vendored
...
def scan_text(text: str) -> list[Threat]:
    ...
    if not text:
        return []
    out: list[Threat] = []
    # prompt injection: reuse rvnd.lock.injection_scan — its Findings become Threats.
