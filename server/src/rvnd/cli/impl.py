...
def _doctor_check_python_binding() -> dict:
    ...
    try:
        from rvnd import __version__ as _our_version  # noqa: WPS433
    ...
    for script in scripts:
        ...
        try:
            probe = _subprocess.run(
                [py_path, "-c", "import rvnd; print(rvnd.__version__)"],
                capture_output=True,
                text=True,
                timeout=10,
            )
        ...
        if probe.returncode != 0:
            ...
            findings.append(
                f"{script} -> {py_path}: cannot import rvnd ({tail}). "
...
def cmd_lens(args: argparse.Namespace) -> int:
    ...
    from rvnd import lens_service as _ls
...
def cmd_grounding(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        print(f"error: {e}", file=sys.stderr); return 3
    if sub == "feed":
        from rvnd.governance import grounding_feed
