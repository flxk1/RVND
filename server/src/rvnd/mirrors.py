...
def _extract_spans_with_lock(text: str) -> tuple[str, list[Span]]:
    ...
    Strategy: ask rvnd.lock to redact the text, then walk the *redaction
    markers* in the cleaned output back against the original to recover the
    span byte offsets. rvnd.lock does NOT expose per-finding offsets in
...
def _diff_spans(original: str, cleaned: str,
    ...
    (approximation — rvnd.lock doesn't expose per-region attribution
    in 0.6.8).
    """
    spans: list[Span] = []
    default_kind = (
