...
1. **connection algebra** (the historical ``legal_connection`` cut) — re-exports
   ...
   ``rvnd.legal_connection`` (the historical shim) consumes them through
...
modules:
  ...
  * **the RVND-KG projection stays local.** ``WorldMap.project`` /
    ...
    ``rvnd.legal_world`` (the shim), not defined here.
...
# packs. Re-exported as the plane's own modules so the ``rvnd.source_classes``
# / ``rvnd.legal_systems`` shims consume them HERE — the single legal import
...
class WorldMap:
    ...
    ``rvnd.legal_world`` (they emit RVND's 5D pair-dict schema, not the
    package's concern)."""
