...
This module is the IR plus:
  ...
  * ``project_to_workspace_workflow`` — the initial projector (IR → the existing
    ``workspaces.workflows.Workflow``). Lossy where the engine is narrower than the
...
class CapabilityStep:
    """One node of a capability. Superset of workspaces.workflows.WorkflowStep."""
...
# ---------------------------------------------------------------------------
# Projector 1 — CapabilitySpec -> workspaces.workflows.Workflow (lossy, reported)
# ---------------------------------------------------------------------------
...
class ProjectionResult:
    ...
    workflow: dict[str, Any]                    # workspaces.workflows.Workflow dict shape
