...
def main(argv: list[str] | None = None) -> int:
    ...
    if cmd in ("setup", "init", "wizard"):
        from workspaces.lock.onboarding.wizard import main as wizard_main
        return wizard_main()

    if cmd in ("mcp", "serve"):
        from workspaces.lock.mcp_server import main as mcp_main
        mcp_main()
        return 0
    ...
    if cmd in ("proxy", "egress"):
        from workspaces.lock.egress_proxy import main as proxy_main
...
def _doctor() -> int:
    ...
    import os
    from workspaces.lock.onboarding.config import load_config, default_config_path, apply_config_to_env
    from workspaces.lock.tier_c import describe_tier_c, is_tier_c_available
