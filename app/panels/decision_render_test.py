...
"""Real render test for the decision workbench (Pending section).

Boots serve.py, then runs decision_render.mjs: builds a surface through the
real decision_build op, walks the workbench — refusals, earned considered,
the recorded chat exchange, the signed receipt — and the single-reading
warning and empty state.

  python3 app/decision_render_test.py
"""
...
def main() -> int:
    ...
    try:
        r = subprocess.run(["node", str(HERE / "decision_render.mjs"), str(PORT), F],
