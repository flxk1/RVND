...
"""Render test for the Policy lock-mode write (workspace_policy set_lock_mode): lock on
(strict default), then the drawer shows 3 discrete modes; tighten is direct and a
loosen step-down needs accepted_by+reason before it writes through.

  python3 app/policy_write_render_test.py
"""
from __future__ import annotations
import os, sys, time, tempfile, threading, subprocess
from pathlib import Path
HERE = Path(__file__).parent
tmp = tempfile.mkdtemp(prefix="polwrite_")
...
def main() -> int:
    S.workspace_policy("enable", {"folder_context": F, "dial": "lock"})
    ...
    try:
        r = subprocess.run(["node", str(HERE / "policy_write_render.mjs"), str(PORT), F],
