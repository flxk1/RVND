...
"""Real render test for the AI & Capture drawer (workspace_model / workspace_capture /
workspace_dispatch, read-only). Asserts the three section headers render, an honest
Models state (no LLM backend in the test env → unreachable/empty), no dial, the
read-only honesty line, and a read-only modal (actions deferred → zero <button>).

  python3 app/ai_render_test.py
"""
...
def main() -> int:
    ...
    time.sleep(0.3)
    try:
        r = subprocess.run(["node", str(HERE / "ai_render.mjs"), str(PORT), F],
