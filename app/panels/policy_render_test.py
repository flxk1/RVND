...
"""Real render test for the Policy drawer (workspace_policy protections).

Boots serve.py against a folder with one registered party, then runs
policy_render.mjs which loads the actual index.html in jsdom, opens the policy
settings, and asserts the 6 discrete oversight levels render, the governed
turn-off requires accepted_by+reason (no bypass) before it writes through, the
parties list renders, and the panel is a modal dialog.

  python3 app/policy_render_test.py
"""
...
def main() -> int:
    ...
    time.sleep(0.3)
    try:
        r = subprocess.run(["node", str(HERE / "policy_render.mjs"), str(PORT), F],
