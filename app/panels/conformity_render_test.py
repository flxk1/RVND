...
"""Real render test for the Conformity drawer (workspace_conformity, read-only).

Boots serve.py with one registered use case, then runs conformity_render.mjs:
opens the Conformity drawer, asserts the six projections render, the regime
selector re-projects with attributed (not asserted) labels, the "does not
certify compliance" line shows, no "compliant" claim appears, and the panel is
a read-only modal.

  python3 app/conformity_render_test.py
"""
...
def main() -> int:
    ...
    time.sleep(0.3)
    try:
        r = subprocess.run(["node", str(HERE / "conformity_render.mjs"), str(PORT), F],
