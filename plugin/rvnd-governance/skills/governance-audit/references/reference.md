...
- `rvnd-audit` verifies one row deeply (the signed chain, receipt-by-receipt) and appends an
  audit-of-audit event; this skill reports the *whole* board and appends nothing. Call `rvnd-audit` when
  a proof row needs receipt-level verification.
- `rvnd-incident` owns the response when the board turns up an `observed_not_authorised` or a broken
  chain. This skill surfaces; the incident skill acts.
- `rvnd-govern` / `rvnd-decide` produce the receipts this board reads.
