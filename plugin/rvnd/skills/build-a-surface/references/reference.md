...
It is the **engine-independent surface builder**: the one skill in this plugin that needs no
local-first cascade (see `../../references/ingest-cascade.md`), because it lowers no text and asks
no engine for a verdict. The surface it builds is what *renders* the shared Subgraph and the
verdicts the cascading skills produce.
...
Composes the cards that `govern-an-action`, `sign-off`, `verify-a-receipt`, and `revoke-or-erase`
drive. It is the one skill here that does not touch the server — it makes sure the surfaces that do
are built honestly.
