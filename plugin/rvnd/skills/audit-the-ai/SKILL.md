...
nothing to the signed chain (unlike `verify-a-receipt`, which records an audit-of-audit event). It
...
## The cascade — this skill's two versions *are* a local-first read

Every skill follows the local-first cascade in `../../references/ingest-cascade.md`: try the engine
first, fall back only when it is absent. For a **pure-read** board that cascade is exactly the two
versions. **With RVND** reads the engine's own signed board first (precise, adjudicated, witnessed).
**Without RVND** falls back to what the host can declare alone (model, tools, coarse sources, declared
tokens) — blind to everything the engine would have signed. Engine board first, host-declarable
fallback; never the reverse. This skill only *reads* the cascade — it enriches nothing and appends
nothing, so the fallback is a narrower view, not a degraded write.
